  ...
  <Dropdown ref="dropdown" @on-click="handleClick" :class="hideTitle ? '' : 'collased-menu-dropdown'" :transfer="hideTitle" :placement="placement">
    ...
    <DropdownMenu ref="dropdown" slot="list">
      <template v-for="(child, index) in children">
        ...
        <DropdownItem v-else :key="`drop-else-${index}`" :name="child.name"><common-icon :size="iconSize" :type="child.icon"/><span class="menu-title">{{ showTitle(child) }}</span></DropdownItem>
      </template>
    </DropdownMenu>
  </Dropdown>
</template>
